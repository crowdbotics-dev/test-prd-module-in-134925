...
class TwoFactorAuthenticationTestCase(APITestCase):
    def setUp(self):
        self.two_fa_user = User.objects.create_user(
            username="john",
            email="fabelet661@semonir.com",
            phone_number="+447520662353",
            password="john123@",
        )
        self.enable_two_fa_user = EnableTwoFactorAuthentication.objects.create(
            user=self.two_fa_user, method="email"
        )
        ...
        self.enable_two_fa_new_user = EnableTwoFactorAuthentication.objects.create(
            user=self.new_user, method="phone_number"
        )
        ...
        self.simple_user = User.objects.create_user(
            username="william", phone_number="+447520662353", password="william123@"
        )
    ...
    def test_send_otp_with_email(self, send_otp_with_email_mock):
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION="Token " + self.two_fa_user_token.key
        )
    ...
    def test_send_otp_with_phone_number(self, send_otp_with_phone_number_mock):
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION="Token " + self.enable_two_fa_new_user_token.key
        )
        url = "/modules/two-factor-authentication/send/otp"
    ...
    def test_send_otp_without_two_fa_enabled_user(self):
        self.client.credentials(
            HTTP_AUTHORIZATION="Token " + self.simple_user_token.key
        )
        url = "/modules/two-factor-authentication/send/otp"
        data = {"method": "email"}
        response = self.client.post(url, data, format="json")
        self.assertEqual(
            response.data, {"message": "Two factor authentication is not enabled"}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_send_otp_with_two_fa_enabled_user_and_invalid_method(self):
        ...
        response = self.client.post(url, data, format="json")
        self.assertEqual(
            response.data, {"message": "You have not selected valid method"}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class GoogleAuthenticatorTestCase(APITestCase):
    def setUp(self):
        ...
        self.user_enable_two_fa = EnableTwoFactorAuthentication.objects.create(
            user=self.user, method="google_authenticator"
        )
        self.user_token = Token.objects.create(user=self.user)
        self.user_token.save()
        self.simple_user = User.objects.create_user(
            username="john", phone_number="+447520662393", password="john123@"
        )
        self.two_fa_user = TwoFactorAuth.objects.create(
            user=self.user, method="email", code="112233"
        )
        self.simple_user_token = Token.objects.create(user=self.simple_user)
        self.simple_user_token.save()
        self.test_invalid_user = User.objects.create_user(
            username="alan_walker", phone_number="+447520662393", password="alan123@"
        )
        self.enable_two_fa_test_invalid_user = (
            EnableTwoFactorAuthentication.objects.create(
                user=self.test_invalid_user, method="email"
            )
        )
    ...
    def test_get_link_with_google_authenticator(
        self, get_link_with_google_authenticator_mock
    ):
        Response = {
            "data": {
    ...
    def test_get_google_authenticator_link_by_enabled_two_fa_user_with_invalid_method(
        self,
    ):
        self.client.credentials(
            HTTP_AUTHORIZATION="Token " + self.test_invalid_user_token.key
        )
        url = "/modules/two-factor-authentication/google/authenticator/qr"
        response = self.client.get(url, format="json")
        self.assertEqual(
...
class OTPVerificationTestCase(APITestCase):
    def setUp(self):
        ...
        self.user_enable_two_fa = EnableTwoFactorAuthentication.objects.create(
            user=self.test_user, method="email"
        )
        self.test_user_token = Token.objects.create(user=self.test_user)
        self.test_user_token.save()
        self.new_user = User.objects.create_user(
            username="devil", email="devil24@gmail.com", password="devil123@"
        )
        self.two_fa_user = TwoFactorAuth.objects.create(
            user=self.new_user, method="email", code=112233
        )
    ...
    def test_verify_otp_with_google_authenticator(
        self, verify_otp_with_google_authenticator_mock
    ):
        ...
        data = {"method": "google_authenticator", "code": 123213}
        response = self.client.post(url, data, format="json")
    ...
    def test_verify_expired_otp_with_google_authenticator(self):
        ...
        url = "/modules/two-factor-authentication/verify/otp"
        data = {"method": "google_authenticator", "code": 123345}
        response = self.client.post(url, data, format="json")
    ...
    def test_verify_otp_with_email(
        self, verify_otp_with_email_mock, verify_send_email_mock
    ):
        Responses = {
            "message": "Verification code has been sent to your Email Address",
            "status": 200,
    ...
    def test_verify_otp_with_invalid_user(self):
        url = "/modules/two-factor-authentication/verify/otp"
        data = {"method": "phone_number", "code": 112233}
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class EnableTwoFactorAuthTestCase(APITestCase):
    def setUp(self):
        ...
        self.enable_two_fa_user = EnableTwoFactorAuthentication.objects.create(
            user=self.new_user, method="email"
        )
        self.two_fa_token = Token.objects.create(user=self.new_user)
        self.two_fa_token.save()
    ...
    def test_enable_two_factor_authentication_by_email(
        self, enable_two_factor_authentication_mock_by_email
    ):
        Response = {
            "message": "Verification code has been sent to your Email Address",
            "status": 200,
    ...
    def test_enable_two_factor_authentication_by_phone_number(
        self, enable_two_factor_authentication_by_phone_number_mock
    ):
        mock_message_create = (
            enable_two_factor_authentication_by_phone_number_mock.messages.create
        )
        Response = {
            "message": "Verification code has been sent to your Phone number",
            "status": 200,
    ...
    def test_enable_link_with_google_authenticator(
        self, enable_link_with_google_authenticator
    ):
        Response = {
            "data": {
                "link": "otpauth://totp/2FA:fabelet661%40semonir.com?secret=3232323232323232&issuer=2FA"
    ...
    def test_disable_two_factor_authentication(self):
        ...
        self.assertEqual(
            response.data, {"message": "Two Factor Authentication disable Successfully"}
        )
