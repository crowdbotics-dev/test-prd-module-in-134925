...
from .serializers import (
    ...
    TwoFactorAuthValidationSerializer,
...
class TwoFactorAuthViewSet(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            user = request.user
            validate_data = {"user": user.id, "method": data.get("method")}
            ...
            try:
                if user.enable_user and user.enable_user.method == data.get("method"):
                    response = TwoFactorAuthenticationService.send_otp(
                        user=request.user, method=validate_data["method"]
                    )
...
class GoogleAuthenticatorViewSet(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if (
                user.enable_user
                and user.enable_user.method == TwoFactorAuth.GOOGLE_AUTHENTICATOR
            ):
...
class EnableTwoFactorAuthViewSet(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            if user.enable_user:
                return Response(
                    {"user": user.username, "method": user.enable_user.method},
    ...
    def post(self, request):
        ...
        try:
            ...
            if EnableTwoFactorAuthentication.objects.filter(
                ~Q(method=validate_data["method"]), user=user
            ).exists():
                EnableTwoFactorAuthentication.objects.update(
                    method=validate_data["method"], user=user
                )
    ...
    def delete(self, request):
        ...
        try:
            user = self.request.user
            EnableTwoFactorAuthentication.objects.get(user=user.id).delete()
            return Response(
